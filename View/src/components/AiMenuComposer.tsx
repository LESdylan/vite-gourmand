...
import {
  Send, Bot, Sparkles, FileText, GripVertical,
  CheckCircle, AlertTriangle, Loader2, X, Maximize2, Minimize2,
  Users, Euro, Calendar, UtensilsCrossed, Leaf, ChefHat,
} from 'lucide-react';
...
const BRIEF_FIELDS: { key: keyof BriefData; label: string; icon: typeof Users; placeholder: string; required: boolean }[] = [
  { key: 'eventType', label: 'Type d\'événement', icon: Sparkles, placeholder: 'Mariage, anniversaire, séminaire…', required: true },
  { key: 'guestCount', label: 'Nombre de convives', icon: Users, placeholder: 'Ex : 80 personnes', required: true },
  { key: 'budget', label: 'Budget par personne', icon: Euro, placeholder: 'Ex : 45€/personne', required: true },
  { key: 'date', label: 'Date souhaitée', icon: Calendar, placeholder: 'Ex : 15 juin 2026', required: true },
  { key: 'dietaryNeeds', label: 'Régimes alimentaires', icon: Leaf, placeholder: 'Végétarien, halal, sans gluten…', required: false },
  { key: 'allergies', label: 'Allergies', icon: AlertTriangle, placeholder: 'Fruits à coque, crustacés…', required: false },
  { key: 'preferences', label: 'Préférences & thème', icon: UtensilsCrossed, placeholder: 'Thème élégant, cuisine du sud-ouest…', required: false },
];
...
export default function AiMenuComposer({
  ...
  const sendMessage = useCallback(async (text?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: '⚠️ Erreur de communication. Veuillez réessayer.' }]);
    } finally {
      setLoading(false);
    }
  ...
  const autoExtractBrief = useCallback((userMsg: string, aiMsg: string) => {
    ...
    setBrief(prev => {
      ...
      if (!next.eventType) {
        const re = /(mariage|anniversaire|s[eé]minaire|bapt[eê]me|communion|f[eê]te|gala|soir[eé]e|cocktail|enterrement de vie|team.?building|repas d'affaire)/i;
        const evtMatch = re.exec(combined);
        if (evtMatch) next.eventType = evtMatch[1].charAt(0).toUpperCase() + evtMatch[1].slice(1);
      }

      if (!next.guestCount) {
        const guestMatch = /(\d+)\s*(personnes|convives|invit[eé]s|pers\b|couverts)/i.exec(combined);
        if (guestMatch) next.guestCount = guestMatch[1] + ' personnes';
      }

      if (!next.budget) {
        const budgetMatch = /(\d+)\s*[€e](?:uros?)?\s*(?:\/|par)\s*(?:pers|personne|convive)/i.exec(combined);
        if (budgetMatch) {
      ...
      if (!next.date) {
        const dateMatch = /(\d{1,2}\s+(?:janvier|f[eé]vrier|mars|avril|mai|juin|juillet|ao[uû]t|septembre|octobre|novembre|d[eé]cembre)\s*\d{0,4})/i.exec(combined);
        if (dateMatch) next.date = dateMatch[1];
      }

      if (!next.dietaryNeeds) {
        const dietMatch = /(v[eé]g[eé]tarien|v[eé]gan|sans gluten|halal|casher|pescetarien)/i.exec(combined);
      ...
      if (aiMsg.includes('MENU') && (aiMsg.includes('ENTRÉE') || aiMsg.includes('PLAT') || aiMsg.includes('convives'))) {
  ...
  const handleSubmitTicket = useCallback(async () => {
    ...
    try {
      ...
      const raw = await apiRequest<
        { data: { ticket_number: string } } | { ticket_number: string }
      >('/api/contact', {
        method: 'POST',
        body: {
          name: contactName,
    ...
    return (
      <div className="bg-gradient-to-br from-[#FFF8F0] to-white rounded-2xl border border-[#556B2F]/20 p-8 text-center">
        ...
        <p className="text-sm text-[#1A1A1A]/60 mb-4">
          Notre équipe a reçu votre brief et vous répondra sous 24h avec une proposition personnalisée.
        </p>
        {ticketNumber && (
          <div className="inline-flex items-center gap-2 bg-white border border-[#D4AF37]/30 rounded-xl px-5 py-3 mb-4">
            <span className="text-[10px] text-[#1A1A1A]/40 uppercase tracking-wider font-semibold">Ticket</span>
    ...
    <div className="space-y-0">
      ...
      >
        ...
        >
          ...
          <div ref={chatScrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#faf8f6]">
            ...
            {messages.length === 0 && !loading && (
              <div className="text-center py-4">
                ...
                <p className="text-sm font-semibold text-[#1A1A1A]/70 mb-1">Composons votre menu ensemble</p>
                <p className="text-xs text-[#1A1A1A]/40 leading-relaxed max-w-[260px] mx-auto mb-4">
                  Décrivez votre événement et je vous proposerai un menu sur mesure parmi nos créations.
                </p>
            ...
            {/* Message bubbles */}
            {messages.map((msg, i) => (
              <div key={`${msg.role}-${i}-${msg.content.slice(0, 16)}`} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
          ...
          <div className="border-t border-[#1A1A1A]/5 p-3 flex gap-2 bg-white shrink-0">
            ...
            >
              <Send className="h-4 w-4" style={{ color: input.trim() && !loading ? 'white' : '#999' }} />
            </button>
          </div>
        </div>
        ...
        >
          ...
          <div className="bg-gradient-to-r from-[#D4AF37]/10 to-transparent px-4 py-3 flex items-center justify-between shrink-0 border-b border-[#D4AF37]/10">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-[#1A1A1A]/40 truncate">
                  {isBriefReady ? '✅ Prêt à envoyer' : `${filledRequired}/${requiredFields.length} champs requis`}
                </p>
              </div>
            </div>
            <button
            ...
            >
              {briefExpanded ? <Minimize2 className="h-3.5 w-3.5 text-[#1A1A1A]/40" /> : <Maximize2 className="h-3.5 w-3.5 text-[#1A1A1A]/40" />}
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
              ...
              return (
                <div key={field.key} className="group">
                  <label className="flex items-center gap-1.5 mb-1">
                    <Icon className={`h-3 w-3 ${filled ? 'text-[#556B2F]' : field.required ? 'text-[#722F37]/50' : 'text-[#1A1A1A]/25'}`} />
                    <span className={`text-[11px] font-semibold ${filled ? 'text-[#1A1A1A]/70' : 'text-[#1A1A1A]/40'}`}>
            ...
            {brief.aiProposal && (
              <div className="mt-2">
                <label className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[11px] font-semibold text-[#1A1A1A]/70">Proposition de l'IA</span>
                </label>
            ...
            <div>
              <label className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[11px] font-semibold text-[#1A1A1A]/40">Notes additionnelles</span>
              </label>
              <textarea
          ...
          <div className="border-t border-[#D4AF37]/10 p-3 bg-white shrink-0">
            {!showContactForm ? (
              ...
              >
                ...
                {isBriefReady ? 'Envoyer la demande' : `Complétez le brief (${filledRequired}/${requiredFields.length})`}
              </button>
            ) : (
              /* Contact info form */
              <div className="space-y-2">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button type="button" onClick={() => setShowContactForm(false)} className="text-[#1A1A1A]/30 hover:text-[#1A1A1A]/60">
      ...
      {briefExpanded && (
        <div
          ...
          onKeyDown={e => { if (e.key === 'Enter' || e.key === 'Escape') setBriefExpanded(false); }}
        />
      )}
    </div>
  );
